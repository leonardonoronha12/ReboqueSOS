...
export async function POST(request: Request) {
  ...
  if (!dbUrl) {
    return NextResponse.json(
      { error: "Configure SUPABASE_DB_URL ou (SUPABASE_PROJECT_REF + SUPABASE_DB_PASSWORD)." },
      { status: 400 },
    );
  }
  ...
  if (dbUrl.startsWith("http://") || dbUrl.startsWith("https://")) {
    return NextResponse.json(
      { error: "SUPABASE_DB_URL não é o Project URL (https://...). Use o Postgres URI (postgresql://...)." },
      { status: 400 },
    );
  }

  const sqlPath = path.join(process.cwd(), "supabase", "migrations", "0005_cancel.sql");
