...
const sqlPath = path.join(root, "supabase", "migrations", "0006_destination.sql");
...
try {
  ...
  const q = `
    ...
    where table_schema='public'
      ...
      and column_name in ('destino_local','destino_lat','destino_lng')
    order by column_name;
  `;
  const { rows } = await client.query(q);
  const cols = rows.map((r) => r.column_name).join(", ");
  console.log("tow_requests destination columns:", cols || "(none)");
} catch (e) {
