...
const sqlPath = path.join(root, "supabase", "migrations", "0009_partner_mercadopago_oauth.sql");
...
try {
  ...
  const q = `
    ...
    where table_schema='public'
      and table_name='tow_partners'
      and column_name in ('mp_user_id','mp_access_token','mp_refresh_token','mp_token_expires_at')
    order by column_name;
  `;
  const { rows } = await client.query(q);
  console.log("tow_partners mp columns:", rows);
} catch (e) {
