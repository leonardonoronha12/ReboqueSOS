...
const sqlPath = path.join(root, "supabase", "migrations", "0007_ratings_guest.sql");
...
try {
  ...
  const q = `
    ...
    where table_schema='public'
      and table_name='tow_ratings'
      and column_name in ('cliente_id','driver_id')
    order by column_name;
  `;
  const { rows } = await client.query(q);
  console.log("tow_ratings columns:", rows);
} catch (e) {
