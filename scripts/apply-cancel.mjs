...
const sqlPath = path.join(root, "supabase", "migrations", "0005_cancel.sql");
...
try {
  ...
  const q = `
    ...
    where table_schema='public'
      and table_name='tow_trips'
      and column_name in ('canceled_at','canceled_by_role','canceled_fee_cents','canceled_after_seconds')
    order by column_name;
  `;
  const { rows } = await client.query(q);
  const cols = rows.map((r) => r.column_name).join(", ");
  console.log("tow_trips cancellation columns:", cols || "(none)");
} catch (e) {
